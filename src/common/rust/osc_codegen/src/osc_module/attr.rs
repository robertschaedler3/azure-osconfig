...
// pub(crate) use manufacturer::Manufacturer;
pub(crate) use version::Version;
// pub(crate) use user_account::UserAccount;

/// [`diagnostic::Scope`] of errors for `#[osc_module]` macro.
const ERR: diagnostic::Scope = diagnostic::Scope::ObjectAttr; // TODO:
...
fn parse_property(method: &mut syn::ImplItemFn) -> Option<property::Definition> {
    ...
    // TODO: filter based on access type:
    // - reported
    //   - no arguments (only an optional &self... reciever?)
    //   - required return value
    // - desired
    //   - one argument (plus an optional &self... reciever?)
    //   - optional return value only for indicating success/failure (Result<(), Error>)
    ...
    let arguments = {
        ...
        method
            ...
            .filter_map(|arg| match arg {
                ...
                // syn::FnArg::Typed(arg) => property::MethodArgument::parse(arg, &ERR),
                syn::FnArg::Typed(arg) => Some(arg.ty.as_ref().clone()),
            })
            .collect()
    };

    // TODO: if desired, this should be the type of the only argument
    ...
    Some(property::Definition {
        ...
        access: attr.access.map(SpanContainer::into_inner)?,
    })
}
