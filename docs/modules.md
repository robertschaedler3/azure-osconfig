...
Each module must have its own MIM JSON saved to the [modules/mim/](../modules/mim/) in a JSON file with the same name as the module SO binary.

The MIM JSON schema is at [modules/schema/mim.schema.json](../modules/schema/mim.schema.json).
...
- CommandRunner: two MIM objects, commandArguments (desired) and commandStatus (reported), linked together by a common setting, commandId: [CommandRunner MIM](../modules/mim/commandrunner.json)
- Tpm: three simple reported MIM objects, each containing a single setting: [Tpm MIM](../modules/mim/tpm.json)
...
The MMI header file is [modules/inc/Mmi.h](../modules/inc/Mmi.h)
...
A JSON schema of the MmiGetInfo payload response is at [MmiGetInfo JSON schema](../modules/schema/mmi-get-info.schema.json)
...
The Logging library header file: [common/logging/Logging.h](../common/logging/Logging.h)
...
The Module Test Recipe JSONs are saved under [modules/test/recipes/](../modules/test/recipes/) as JSON files, one for each module, named as module name with a "Tests" suffix ("ModuleNameTests.json"). For example: CommandRunnerTests.json, DeviceInfoTests.json.
...
The Module Test Recipe is fed into the OSConfig's ModulesTest utility to be executed. For more information about ModulesTest and how to run it see [modules/test/README.md](../modules/test/README.md).
